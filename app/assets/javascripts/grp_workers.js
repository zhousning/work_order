$(".grp_workers").ready(function() {
  if ($(".grp_workers.index").length > 0) {
    get_grp_workers('grp_workers', 'query_all')
    grp_worker_bind_event('query_all')
  }
  if ($(".grp_workers.supervisor").length > 0) {
    get_grp_workers('grp_workers', 'query_supervisor')
    grp_worker_bind_event('query_supervisor')
  }
});


function grp_worker_bind_event(link) {
  ...
  $("#item-table").on('click', 'button.worker-show-btn', function(e) {
    ...
    var that = e.target
    var data_id = that.dataset['id'];
    var url = "/grp_workers/" + data_id + "/query_info";
    $.get(url).done(function (data) {
      var emq = data.info;
...
function get_grp_workers(method, link) {
  ...
  $.get(url).done(function (objs) {
    $.each(objs, function(index, item) {
      var id = item.id;
      //var button = "<button class = 'button button-royal button-small mr-1 log-show-btn' type = 'button' data-id ='" + id + "'>签到记录</button><button class = 'button button-primary button-small mr-1 worker-show-btn' type = 'button' data-id ='" + id + "'>证件照</button><button class='button button-caution button-small mr-2 worker-delete-btn' type = 'button' data-id ='" + id + "' data-confirm='确定删除吗?'>删除</button>"
      data.push({
        'id' : index + 1,
